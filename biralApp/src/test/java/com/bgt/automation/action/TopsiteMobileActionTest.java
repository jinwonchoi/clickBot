...
public class TopsiteMobileActionTest {
	...
	void testAction(String browser, String site, ActionItem item) {
		TopsiteMobileAction action = null;
		try {
...
//		testAction(CommonConst.BROWSER_IEXPLORER.get(), CommonConst.SITE_DAUM.get(), daumBlog04());
	...
	ActionItem naverBlog01() {
		...
		item.setTargetUrl("http://www.lagopension.co.kr/");
	...
	ActionItem daumBlog02() {
		...
		item.setKeyword2("철쭉");
		item.setTargetUrl("http://www.esa.int/Our_Activities/Space_Science/Rosetta/");
		item.setActionType(CommonConst.TASK_TOPSITE.get());
		item.setDeviceType(CommonConst.DEVICE_MOBILE.get());
		return item;
	}
