...
public class AutoCompleteMobileAction extends BaseAction {

	static Logger Log = Logger.getLogger(AutoCompleteMobileAction.class);

	public AutoCompleteMobileAction(ActionItem item)
			throws Exception {
		super(item);
		if (!CommonConst.DEVICE_MOBILE.get().equals(item.getDeviceType()) )
				throw new Exception("모바일 작업인데 웹브라우저로 설정.");
